...
public class ICode {
	...
	while (scanner.hasNext()) {
		...
		if (mainMethod != null)
		    throw new ICodeException("Multiple main methods specified");
	...
	if (declaredSymbols.contains(s))
	    throw new ICodeException ("Multiple declarations of " + s);
	...
	Exp e = expMap.get(s);
	if (e == null)
	    throw new ICodeException ("Using undefined expression " + s);
	return e;
    }
	...
	Stm e = stmMap.get(s);
	if (e == null)
	    throw new ICodeException ("Using undefined statement " + s);
	return e;
    }
	...
	while (true) {
		...
		throw new ICodeException("Unexpected kind of node (" + kind + ") in intermediate tree");
	}
	...
	Simulator sim = new Simulator(stringMap, globalVars, methods, labelMap, mainMethod);
	sim.simulate();
    }
