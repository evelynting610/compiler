...
/**
 * A Print object permits statements or expressions to be printed with appropriate indentation.
 */
...
public class Print {
  ...
  TempMap tmap;
    ...
    /**
       Class constructor
       @param o PrintStream to which output is sent
       @param t TempMap to be applied to a Temp when it is printed
    */
  public Print(java.io.PrintStream o, TempMap t) {out=o; tmap=t;}

    /**
       Class constructor that prints based on a default TempMap
       @param o PrintStream to which output is sent
    */
  ...
  void prStm(JUMP s, int d) {
      indent(d); sayln("JUMP "); say(s.target.toString());
  }
  ...
  void prStm(ESTM s, int d) {
     indent(d); sayln("ESTM("); prExp(s.exp,d+1); say(")"); 
  }
  ...
  void prStm(Stm s, int d) {
   ...
   else if (s instanceof ESTM) prStm((ESTM)s, d);
